...
risk_functionals = {
    ...
    'CVaR': cvar.CVaR(a=0.2),
    'Entropic Risk': entropic_risk.EntropicRisk(t=-2),
    'Human-Aligned Risk': human_aligned_risk.HumanAlignedRisk(a=0.4, b=0.8),
    'Inverted CVaR': cvar.CVaR(a=0.2, inverted=True),
    'Mean-Variance': mean_variance.MeanVariance(c=1),
    'Trimmed Risk': trimmed_risk.TrimmedRisk(a=0.4),
}
...
for risk in risk_functionals.keys():
    ...
    os.makedirs(str(risk), exist_ok=True)
    os.makedirs(str(risk) + '/checkpoint', exist_ok=True)
    ...
    for epoch in range(start_epoch, start_epoch+100):
        train(epoch, risk, risk_functionals[risk])
        test(epoch, risk, risk_functionals[risk])
        scheduler.step()
